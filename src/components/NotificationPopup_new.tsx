    ...
    >
      <div className="flex items-center justify-between bg-indigo-600 px-3 py-2">
        <button onClick={() => setExpanded((v) => !v)} className="flex items-center gap-2 text-white text-sm font-semibold hover:text-indigo-100 focus:outline-none flex-1 text-left">
          <span>🔔 Permintaan Sampel Baru</span>
      ...
      {!expanded && (
        <button onClick={() => setExpanded(true)} className="w-full px-4 py-2 text-left hover:bg-gray-50 transition-colors">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{formatDateTimeWib(n.Created_At)}</span>
          </div>
      ...
      {expanded && (
        <div className="px-4 py-3 space-y-2 border-t border-gray-100 bg-gray-50">
          ...
          <a href="/loan-notifications" onClick={onDismiss} className="block text-center text-xs font-semibold text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 rounded py-1.5 mt-1 transition-colors">
    ...
    >
      ...
      {!expanded && (
        <button onClick={() => setExpanded(true)} className="w-full px-4 py-2 text-left hover:bg-gray-50 transition-colors">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">{formatDateTimeWib(n.createdAt)}</span>
          </div>
      ...
      {expanded && (
        <div className="px-4 py-3 space-y-2 border-t border-gray-100 bg-gray-50">
          ...
          <a href="/loan-notifications" onClick={onDismiss} className="block text-center text-xs font-semibold text-emerald-600 hover:text-emerald-800 bg-emerald-50 hover:bg-emerald-100 rounded py-1.5 mt-1 transition-colors">
            Lihat Notifikasi Pengembalian →
          </a>
        </div>
      )}
    </div>
